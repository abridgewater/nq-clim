;;;
;;; nq-clim/geometry/shape-operations.lisp
;;;
;;; The "shape" abstraction for composite rectilinear regions.
;;;

(cl:defpackage :nq-clim/geometry/shape-operations
  (:use :cl)
  (:export
   "UNITE-Y-SPAN-SETS"
   "INTERSECT-Y-SPAN-SETS"
   "DIFFER-Y-SPAN-SETS"))
(cl:in-package :nq-clim/geometry/shape-operations)
          ...
          (macrolet ((discard-span-before (span boundary)
            ...
            (loop
               ...
               do (cond
                    ((< start-1 start-2)
                     (when (funcall operation t nil)
                       (collect-y-span start-1 (min end-1 start-2)
                                       x-spans-1))
                     (discard-span-before span-1 start-2))
...
(defun intersect-y-span-sets (set-1 set-2)
  "Given two sets of Y-spans, each in order, produce a minimal set of
Y-spans representing the intersection of both sets."
  (operate-on-y-span-sets set-1 set-2 #'intersection-operation))
